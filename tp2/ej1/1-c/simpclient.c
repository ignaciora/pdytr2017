...
int subUDP( CLIENT *clnt, int x, int y) {
  operands ops;
  int *result;

  /* >>>>> MODIFICA EL VALOR HOST DEL PUNTERO AL CLIENTE <<<<< */
  clnt = clnt_create("127.0.0.2", SIMP_PROG, SIMP_VERSION, "udp");
  printf("Host no valido, probando por UDP...\n");
...
/* >>>>> SE DUPLICA LA FUNCION PARA PODER PROBAR POR TCP <<<<< */
int subTCP( CLIENT *clnt, int x, int y) {
  operands ops;
  int *result;

  /* >>>>> MODIFICA EL VALOR HOST DEL PUNTERO AL CLIENTE Y CAMBIA A TCP <<<<< */
  clnt = clnt_create("127.0.0.3", SIMP_PROG, SIMP_VERSION, "tcp");
  printf("Host no valido, probando por TCP...\n");
